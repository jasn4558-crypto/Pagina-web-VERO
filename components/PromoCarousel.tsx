...
export default function PromoCarousel({ items }: PromoCarouselProps) {
  ...
  return (
    <section className="relative border-b border-stone-200 bg-white py-4 select-none">
      {/* Título */}
      <div className="mb-3 flex items-center justify-center gap-2">
        <Tag className="h-4 w-4 text-emerald-600" />
        <span className="text-xs font-extrabold uppercase tracking-widest text-stone-800 sm:text-sm">
          Ofertas Especiales
        </span>
        <Tag className="h-4 w-4 text-emerald-600" />
      </div>
      ...
      {!shouldScroll ? (
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-center gap-3 px-4 sm:gap-4">
          {items.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="flex w-[180px] shrink-0 flex-col items-center gap-2 rounded-2xl border border-stone-100 bg-stone-50 p-3 shadow-sm transition duration-200 hover:border-emerald-200 hover:shadow-md sm:w-[200px] group"
              >
                {/* Imagen */}
        ...
        /* Si hay 5 o más productos: Carrusel animado con deslizamiento */
        <div className="relative mx-auto max-w-5xl overflow-hidden px-10">
          {/* Fade masks */}
          ...
          <button
            ...
            className="absolute left-1 top-1/2 z-20 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full border border-stone-200 bg-white shadow-md transition hover:bg-stone-50"
            aria-label="Anterior"
          >
          ...
          <button
            ...
            onMouseEnter={() => (pausedRef.current = true)}
            onMouseLeave={() => (pausedRef.current = false)}
            className="absolute right-1 top-1/2 z-20 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full border border-stone-200 bg-white shadow-md transition hover:bg-stone-50"
            ...
            >
              {displayItems.map((item, idx) => {
                ...
                return (
                  <div
                    key={`${item.id}-${idx}`}
                    className="flex w-[200px] shrink-0 cursor-grab active:cursor-grabbing flex-col items-center gap-2 rounded-2xl border border-stone-100 bg-stone-50 p-3 shadow-sm transition hover:border-emerald-200 hover:shadow-md group"
                    draggable={false}
                  >
                    {/* Imagen */}
                    <div className="relative h-24 w-full overflow-hidden rounded-xl bg-stone-100">
